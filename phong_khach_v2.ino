...
byte mac[] = { 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED }; // Địa chỉ MAC của Shield
IPAddress ip(192, 168, 1, 178); // Địa chỉ IP tĩnh cho Ethernet Shield
EthernetServer server(80); // Khởi tạo server tại port 80 (HTTP)
...
void setup() {
  ...
  Serial3.flush();
  //----
  while (!Serial) {} // ham su dung khi ket noi serialport 
  ...
  Serial.print("Địa chỉ IP của thiết bị: ");
  Serial.println(Ethernet.localIP()); // In ra địa chỉ IP trên Serial Monitor
